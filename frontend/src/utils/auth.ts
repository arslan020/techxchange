...
export async function registerUser(payload: {name?:string; email:string; password:string; role?:string}) {
  const data = await api<{ token:string; user:User }>("/auth/register", { method:"POST", body: JSON.stringify(payload) });
...
export async function loginUser(payload: {email:string; password:string}) {
  const data = await api<{ token:string; user:User }>("/auth/login", { method:"POST", body: JSON.stringify(payload) });
  localStorage.setItem("txc_token", data.token);
  localStorage.setItem("txc_user", JSON.stringify(data.user));
  return data.user;
}
